...
// OrganizationTagResponse 隔离 API 响应与 ORM 模型。
type OrganizationTagResponse struct {
...
func organizationTagResponse(tag model.OrganizationTag) OrganizationTagResponse {
	return OrganizationTagResponse{
		TagID: tag.TagID, Name: tag.Name, Description: tag.Description,
		ParentTag: tag.ParentTag, CreatedBy: tag.CreatedBy,
...
func (h *AdminHandler) CreateOrganizationTag(c *gin.Context) {
	...
	// 获取完整的创建者用户信息
	creator, err := h.userService.GetProfile(c.Request.Context(), claims.Username)
	if err != nil {
	...
	c.JSON(http.StatusOK, gin.H{"code": http.StatusOK, "message": "success", "data": organizationTagResponse(*tag)})
}
...
func (h *AdminHandler) ListOrganizationTags(c *gin.Context) {
	...
	responses := make([]OrganizationTagResponse, 0, len(tags))
	for _, tag := range tags {
		responses = append(responses, organizationTagResponse(tag))
	}
	c.JSON(http.StatusOK, gin.H{"code": http.StatusOK, "message": "success", "data": responses})
}
...
func (h *AdminHandler) GetAllConversations(c *gin.Context) {
	...
	if value := c.Query("page"); value != "" {
		...
		if err != nil || parsed < 1 {
			c.JSON(http.StatusBadRequest, gin.H{"code": http.StatusBadRequest, "message": "Invalid page", "data": nil})
			return
		}
		page = parsed
	}
	if value := c.Query("size"); value != "" {
		...
		if err != nil || parsed < 1 || parsed > 100 {
			c.JSON(http.StatusBadRequest, gin.H{"code": http.StatusBadRequest, "message": "Invalid size (1..100)", "data": nil})
			return
		}
		size = parsed
	}
	...
	conversations, err := h.adminService.GetAllConversations(c.Request.Context(), userID, startTime, endTime, page, size)
	if err != nil {
...
func (h *AdminHandler) UpdateOrganizationTag(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"code": http.StatusOK, "message": "success", "data": organizationTagResponse(*updatedTag)})
}
