...
func (h *UploadHandler) UploadChunk(c *gin.Context) {
	// Bound the multipart body before PostForm/FormFile can buffer or spill it.
	c.Request.Body = http.MaxBytesReader(c.Writer, c.Request.Body, service.DefaultChunkSize+(1<<20))
	err := c.Request.ParseMultipartForm(service.DefaultChunkSize)
	...
	if err != nil {
		...
		c.JSON(status, gin.H{"code": status, "message": "上传分片过大或表单无效"})
		return
	}
	// 从表单中获取参数
...
func (h *UploadHandler) MergeChunks(c *gin.Context) {
	...
	if err != nil {
		...
		c.JSON(uploadErrorStatus(err), gin.H{"error": "文件合并失败: " + err.Error()})
		return
	}
