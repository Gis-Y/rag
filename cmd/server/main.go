...
func main() {
	...
	if err := cfg.DocumentProcessing.Validate(cfg.Embedding); err != nil {
		panic(err)
	}
	cfg.DocumentProcessing = cfg.DocumentProcessing.WithDefaults()
	if cfg.Elasticsearch.Dimensions > 0 && cfg.Elasticsearch.Dimensions != cfg.Embedding.Dimensions {
		panic("elasticsearch.dimensions must match embedding.dimensions")
	}
	cfg.Elasticsearch.Dimensions = cfg.Embedding.Dimensions
	...
	objectStore, err := storage.NewClient(context.Background(), cfg.MinIO)
	if err != nil {
		log.Fatal("MinIO 初始化失败", err)
		return
	}
	searchIndex, err := es.NewClient(cfg.Elasticsearch)
	if err != nil {
		log.Fatal("Elasticsearch 初始化失败", err)
		return
	}
	taskProducer := kafka.NewProducer(cfg.Kafka)
	defer func() {
		if err := taskProducer.Close(); err != nil {
			log.Warnf("关闭 Kafka Producer 失败: %v", err)
		}
	}()
	...
	conversationRepo := repository.NewConversationRepository(database.RDB, database.DB)
	userMemoryRepo := repository.NewUserMemoryRepository(database.DB)
	if !database.DB.Migrator().HasTable("document_processing_states") || !database.DB.Migrator().HasTable("document_chunks") {
		log.Fatal("文档处理表缺失", fmt.Errorf("请先初始化 docs/ddl.sql；已建开发库可执行 docs/migrations/20260903_document_processing.sql 补齐文档表"))
		return
	}
	processingRepo := repository.NewDocumentProcessingRepository(database.DB)
	if !database.DB.Migrator().HasColumn("conversations", "sources") {
		log.Fatal("会话来源字段缺失", fmt.Errorf("请执行 docs/migrations/20260903_document_sources.sql"))
		return
	}
	if !database.DB.Migrator().HasTable("document_task_outbox") || !database.DB.Migrator().HasColumn("file_upload", "merge_token") || !database.DB.Migrator().HasColumn("file_upload", "merge_expires_at") || !database.DB.Migrator().HasColumn("chunk_info", "document_id") || !database.DB.Migrator().HasColumn("chunk_info", "user_id") {
		log.Fatal("上传生命周期表结构缺失", fmt.Errorf("请执行 docs/migrations/20260904_document_upload_lifecycle.sql"))
		return
	}
	outboxRepo := repository.NewDocumentTaskOutboxRepository(database.DB)
	dispatchCtx, cancelDispatch := context.WithCancel(context.Background())
	dispatchDone := make(chan struct{})
	go func() {
		defer close(dispatchDone)
		service.RunDocumentTaskDispatcher(dispatchCtx, outboxRepo, taskProducer)
	}()
	defer func() { cancelDispatch(); <-dispatchDone }()
	...
	uploadService := service.NewUploadService(uploadRepo, userRepository, objectStore, cfg.DocumentProcessing.MaxFileBytes)
	documentService := service.NewDocumentService(uploadRepo, orgTagRepo, userService, objectStore, service.DocumentProcessingOptions{Repository: processingRepo, Index: searchIndex})
	searchService, err := service.NewSearchService(embeddingClient, searchIndex, userService, uploadRepo, service.DocumentSearchOptions{Repository: processingRepo, ModelVersion: cfg.Embedding.Model, ParentContextTokens: cfg.DocumentProcessing.ParentContextTokens})
	if err != nil {
		log.Fatal("文档检索初始化失败", err)
		return
	}
	processingHandler := handler.NewDocumentProcessingHandler(uploadRepo, processingRepo, outboxRepo, userService)
	conversationService := service.NewConversationService(conversationRepo)
	userMemoryService := service.NewUserMemoryService(userMemoryRepo, conversationRepo, cfg.Memory)
	userMemoryHandler := handler.NewUserMemoryHandler(userMemoryService)
	chatService := service.NewChatService(searchService, llmClient, conversationRepo, cfg.AI, cfg.LLM, cfg.QueryUnderstanding, cfg.Memory, userMemoryService)
	...
	processor := pipeline.NewProcessor(
		...
		objectStore,
		searchIndex,
		pipeline.StructuredOptions{Config: cfg.DocumentProcessing, Parser: document.NewClient(cfg.DocumentProcessing), Repository: processingRepo},
	)
	...
	apiV1 := r.Group("/api/v1")
	{
		...
		users := apiV1.Group("/users")
		{
			...
			authed.Use(middleware.AuthMiddleware(jwtManager, userService))
			{
				...
				authed.GET("/memories", userMemoryHandler.List)
				authed.POST("/memories", userMemoryHandler.Save)
				authed.PUT("/memories/:id", userMemoryHandler.Save)
				authed.DELETE("/memories/:id", userMemoryHandler.Delete)
			}
		}
		...
		{
			documents.GET("/:fileMd5/processing", processingHandler.Status)
			documents.POST("/:fileMd5/reprocess", processingHandler.Retry)
			...
			documents.DELETE("/:documentId", handler.NewDocumentHandler(documentService, userService).DeleteDocument)
		...
		conversation.Use(middleware.AuthMiddleware(jwtManager, userService))
		{
			...
			conversation.GET("/archive", handler.NewConversationHandler(conversationService).GetArchive)
		}

		// Chat 路由 (WebSocket)
		chatGroup := apiV1.Group("/chat")
		chatGroup.Use(middleware.AuthMiddleware(jwtManager, userService))
		{
			chatGroup.GET("/websocket-token", chatHandler.GetWebsocketTicket)
		}
		r.GET("/chat/:ticket", chatHandler.Handle)
...
func initSeedFiles(ctx context.Context, dir string, userRepo repository.UserRepository, uploadSvc service.UploadService) {
	...
	var ownerOrg string
	if admin, err := userRepo.FindByUsername(ctx, "admin"); err == nil && admin != nil {
	...
	walkErr := filepath.Walk(dir, func(path string, info os.FileInfo, err error) error {
		...
		for chunkIndex := 0; chunkIndex < totalChunks; chunkIndex++ {
			...
			if _, _, err := uploadSvc.UploadChunk(ctx, fileMD5, fileName, size, chunkIndex, bytes.NewReader(buf), ownerUserID, ownerOrg, true); err != nil {
