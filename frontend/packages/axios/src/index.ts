...
import type { AxiosResponse, CreateAxiosDefaults, GenericAbortSignal, InternalAxiosRequestConfig } from 'axios';
...
function createCommonRequest<ResponseData = any>(
  ...
  const managedSignals = new WeakSet<GenericAbortSignal>();
  ...
  instance.interceptors.request.use(conf => {
    ...
    if (!requestId) {
      requestId = Array.from(crypto.getRandomValues(new Uint8Array(16)), value => value.toString(16).padStart(2, '0')).join('');
      config.headers.set(REQUEST_ID_KEY, requestId);
    }

    // config abort controller
    if (!config.signal || managedSignals.has(config.signal)) {
      const abortController = new AbortController();
      config.signal = abortController.signal;
      managedSignals.add(config.signal);
  ...
  function releaseAbortController(config?: InternalAxiosRequestConfig) {
    const requestId = config?.headers.get(REQUEST_ID_KEY);
    if (typeof requestId === 'string' && requestId) abortControllerMap.delete(requestId);
  }
    ...
    async response => {
      try {
        ...
        if (typeof newToken === 'string' && newToken) {
          ...
          await opts.onTokenRefresh?.(newToken, response.config);
        }
    ...
    async (error: AxiosError<ResponseData>) => {
      try {
        if (error.response?.status === 401) {
          const recovered = await opts.onBackendFail(error.response, instance);
...
export type { CreateAxiosDefaults, AxiosError, AxiosResponse, InternalAxiosRequestConfig };
