...
export const useAuthStore = defineStore(SetupStoreId.Auth, () => {
  ...
  async function resetStore() {
    ...
    // The setup-store reset plugin restores the startup token; never restore a logged-out session.
    token.value = '';
    Object.assign(userInfo, { id: 0, username: '', role: 'USER', orgTags: [], primaryOrg: '' });
    endLoading();
  ...
  async function login(userName: string, password: string, redirect = true) {
    ...
    if (identityVersion !== requestIdentity) return;
  ...
  async function loginByToken(loginToken: Api.Auth.LoginToken) {
    // Drop the previous account's socket and chat before fetching the new user's details.
    token.value = '';
  ...
  async function getUserInfo() {
    ...
    if (identityVersion !== requestIdentity) return false;
  ...
  async function initUserInfo() {
    ...
    if (hasToken) {
      ...
      if (!pass && identityVersion === requestIdentity) {
        resetStore();
      }
    }
  }
