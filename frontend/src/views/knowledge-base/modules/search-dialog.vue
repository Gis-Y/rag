...
async function search() {
  ...
  const query = model.value.query;
  const requestParams = { ...model.value };
  loading.value = true;
  try {
    ...
    list.value = !error && Array.isArray(data) ? data : [];
    patterns.value = !error && query ? [query] : [];
  } finally {
...
watch(() => [store.token, store.userInfo.id], () => {
  reset();
});
onBeforeUnmount(() => {
  searchRequest += 1;
