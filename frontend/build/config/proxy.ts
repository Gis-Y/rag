...
function createProxyItem(item: App.Service.ServiceConfigItem, enableLog: boolean) {
  ...
  const isWebSocketTarget = /^wss?:\/\//.test(item.baseURL);

  proxy[item.proxyPattern] = {
    target: item.baseURL,
    // Keep the browser's original Host so Gorilla's default same-origin check
    // compares Origin against the public Vite host, not the upstream host.
    changeOrigin: !isWebSocketTarget,
